...
export const conversationApi = {
    ...
    getConfiguration: async (conversationId, messageId) => {
        const response = await api.get(`/conversations/${conversationId}/messages/${messageId}/configuration`);
        return response.data;
    },
    
    confirmTurn: async (conversationId, messageId, config) => {
        const response = await api.post(`/conversations/${conversationId}/messages/${messageId}/confirm`, config);
        return response.data;
    },
    
    getAgentOutputs: async (conversationId, messageId) => {
        const response = await api.get(`/conversations/${conversationId}/messages/${messageId}/agent-outputs`);
        return response.data;
    }
};
